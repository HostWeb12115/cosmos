...
% Use standard and robust geometry package for margins
\usepackage[left=1.44in, right=1.63in, top=1in, bottom=1in]{geometry}
\usepackage{amsmath}
% Using xfrac for diagonal fractions is fine, but we'll use \frac{} for key math.
\usepackage{xfrac}
\usepackage{setspace}
\usepackage{url}
\usepackage{tikz}
\usepackage{color}
\usepackage{graphicx} % Use graphicx instead of the older 'graphics'
\usepackage{float}
\usepackage{verbatim}
\usepackage{listings}
\usepackage{alltt}
\usepackage[fleqn]{amsmath} % Use fleqn for left-aligned equations if desired
\usepackage[utf8]{inputenc}
\usepackage[noend]{algorithm2e} % Use noend for cleaner algorithm blocks

% Use a standard way to define abstract environment for better structure
\usepackage{abstract}
\renewcommand{\abstracttextfont}{\normalfont\fontsize{10pt}{12pt}\selectfont}
\setlength{\absleftindent}{0.1\textwidth}
\setlength{\absrightindent}{0.1\textwidth}

% Use titlesec to safely customize section/subsection appearance if needed
\usepackage{titlesec}
\titleformat{\section}{\vspace{9mm}\normalfont\fontsize{13pt}{15pt}\bfseries}{\thesection.}{0.5em}{}
\titlespacing*{\section}{0pt}{0pt}{5mm}
\titleformat{\subsection}[runin]{\vspace{0mm}\normalfont\fontsize{11pt}{13pt}\bfseries}{\thesubsection}{0.5em}{}[.]
\titlespacing*{\subsection}{0pt}{5mm}{1em}

\setstretch{1.1} % Linespacing
\setlength{\parindent}{12mm} % First line paragraph indent value

% Custom command for small code snippets remains, but uses standard font size
\newcommand{\mygrey}[1]{\color{black!60}\small\texttt{#1}\normalsize\color{black}\,}

% Use a proper structure for bibliography labels
\makeatletter
...
\begin{center}
    \vspace*{1cm}
    \fontsize{21pt}{24pt}\selectfont \textbf{Tendermint: Consensus without Mining}
    \vspace{2mm}
    
    \normalsize
    Jae Kwon\\
    \mygrey{yk239@cornell.edu}\\
    Draft v.0.6
\end{center}
...
% --- CUSTOM STRUCT DEFINITION (CLEANED) ---
\SetKwProg{Structure}{Structure}{ contains}{end}
\begin{algorithm}[H]
\Structure{Node}{
  Node* parent\;
  int size\;
...
\caption{Example Data Structure}
\end{algorithm}
...
Our algorithm is based on algorithm 2’ from section 4 of \cite{article:dwork} (Dwork et al). It assumes that the network is partially synchronous; there is assumed to be some unknown upper bound $\Delta$ on the time of messages to be delivered. Intuitively, there may be arbitrary but finite latency in the network. We also assume that all non-byzantine nodes have access to an internal clock that can stay sufficiently accurate for a short duration of time until consensus on the next block is achieved. The clocks do not need to agree on a global time and may drift at some bounded rate relative to global time. The algorithm is adapted to work with blockchains on a gossip network. As in the algorithm proposed by Dwork et al, it can tolerate of up to $\frac{1}{3}$ byzantine voting power.

\subsection{Algorithm}

Validators participate in the consensus process by signing votes for blocks. There are three types of votes: a \textit{prevote}, a \textit{precommit} and a \textit{commit}. To \textit{receive more than $\frac{2}{3}$ of commits} means to receive commits from a $\frac{2}{3}$ majority of validators. A block is said to be \textit{committed by the network} when a $\frac{2}{3}$ majority of validators had signed and broadcast commits for that block.
...
At each height of the blockchain a round-based protocol is run to determine the next block. Each round is composed of three steps (\textit{Propose}, \textit{Prevote}, and \textit{Precommit}), along with two special steps \textit{Commit} and \textit{NewHeight}. The \textit{Propose}, \textit{Prevote}, and \textit{Precommit} steps each take one third of the total time allocated for that round. Each round is longer than the previous round by a small fixed increment of time. This allows the network to eventually achieve consensus in a partially synchronous network.
...
The first step is the \textit{Propose} step. In the beginning of the \textit{Propose} step the designated proposer for that round broadcasts a proposal to its peers via gossip. If the proposer is locked on a block from some prior round it proposes the locked block and includes a \textit{proof-of-lock} in the proposal (more on that later). During the \textit{Propose} step all nodes gossip the proposal to their neighboring peers.

In the beginning of the \textit{Prevote} step each validator makes a decision. If the validator is locked on a proposed block from some prior round, it signs and broadcasts a prevote for the locked block. Otherwise if the validator had received an acceptable proposal for the current round, then it signs and broadcasts a prevote for the proposed block. If the validator had received no proposal or an invalid one, it signs a special \textit{nil} prevote. No locking happens during the \textit{Prevote} step. During the \textit{Prevote} step all nodes gossip all prevotes for the round to their neighboring peers.

In the beginning of the \textit{Precommit} step each validator makes a decision. If the validator had received more than $\frac{2}{3}$ of prevotes for a particular acceptable block then the validator signs and broadcasts a precommit for that block. It also locks onto that block and releases any prior locks. A node has a lock on at most one block at a time. If the node had received more than $\frac{2}{3}$ of \textit{nil} prevotes then it simply unlocks. When locking (or unlocking) the node gathers the prevotes for the locked block (or the prevotes for \textit{nil}) and packages them into a \textit{proof-of-lock} for later when it is its turn to propose. If a node had not received more than $\frac{2}{3}$ of prevotes for a particular block (or \textit{nil}), then it does not sign or lock anything. During the \textit{Precommit} step all nodes gossip all precommits for the round to all neighboring peers.

At the end of the \textit{Precommit} step each node makes a decision. If the node had received more than $\frac{2}{3}$ of precommits for a particular block, then the node enters the \textit{Commit} step. Otherwise it continues onto the \textit{Propose} step of the next round. Even if a node hadn’t yet received the block precommitted by the network, it enters the \textit{Commit} step.

The \textit{Commit} step is a special step. There are two parallel conditions that must both be satisfied before finalizing the round. First, the node must receive the block committed by the network if it hadn’t already. Once the block is received by a validator it signs and broadcasts a commit for that block. Second, the node must wait until it receive at least $\frac{2}{3}$ of commits for the block precommitted by the network. Once both conditions are satisfied the node sets its $CommitTime$ to the current time and transitions to the \textit{NewHeight} step. The asynchronous and local nature of $CommitTime$ allows the network to maintain consensus despite drifting clocks, as long as the clocks remain accurate enough during the consensus process of a given height.

Before the first round (round 0) at height $H$ is the \textit{NewHeight} step whose purpose is to gather additional commits for the previously committed block at height \textit{H-1}. Nodes stay at this step until some fixed duration of time past \textit{CommitTime}. This allows block proposals to include more than the minimum $\frac{2}{3}$ of commits, thus allowing the commits of slower validators to be included in the blockchain.

At any time during the consensus process if a node receives more than $\frac{2}{3}$ of commits for a particular block, it immediately enters the \textit{Commit} step if it hadn’t already. Thus there are two ways to enter the \textit{Commit} step. A commit-vote for a block at round $R$ counts as prevotes and precommits for all rounds $R’$ where $R < R’$. Commit-votes are gossipped to neighboring peers in the background regardless of the current round or step.
...
\section{Incentive Analysis} % Renamed from 'Cooperation'
...
$$E[\text{fees}] \cdot v_a \cdot v_p \cdot v_p$$

whereas P’s expected detriment on each block is:

$$E[\text{fees}] \cdot v_p \cdot v_a$$
...
\subsection{Sparse Signature Set}
...
\subsection{Handling Validator Timeouts}
...
\subsection{Faster Block Propagation}
